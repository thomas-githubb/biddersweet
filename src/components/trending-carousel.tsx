...
export function TrendingCarousel({ items: initialItems }: TrendingCarouselProps) {
  ...
  const toggleLike = (itemId: number) => {
    setItems(currentItems =>
      currentItems.map(item =>
        item.id === itemId
          ? { ...item, isLiked: !item.isLiked, bids: item.isLiked ? item.bids - 1 : item.bids + 1 }
          : item
      )
    );
  };
    ...
    const interval = setInterval(() => {
      setItems(currentItems =>
        currentItems.map(item => {
          ...
          const bidIncrement = Math.floor(item.currentBid * (0.01 + Math.random() * 0.02)); // 1-3% increase
          
          return {
            ...item,
            watchers: newWatchers,
            currentBid: shouldIncreaseBid ? item.currentBid + bidIncrement : item.currentBid,
            ...
            return (
              ...
              >
                <Card className="overflow-hidden hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-purple-900/20 to-gray-900/20 border-purple-900/20 hover:border-purple-500/30">
                  ...
                  <div className="p-4">
                    <h3 className="font-semibold text-lg mb-2 text-purple-100 line-clamp-1">{item.name}</h3>
